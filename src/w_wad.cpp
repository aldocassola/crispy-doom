...
static void AddWADFileName(const char *filename)
{
    ...
    wad_filenames = static_cast<decltype(wad_filenames)>(I_Realloc(wad_filenames, (i + 2) * sizeof(*wad_filenames)));
...
wad_file_t *W_AddFile (const char *filename)
{
	...
	fileinfo = static_cast<decltype(fileinfo)>(Z_Malloc(sizeof(filelump_t), PU_STATIC, 0));
	...
	fileinfo = static_cast<decltype(fileinfo)>(Z_Malloc(length, PU_STATIC, 0));
...
void *W_CacheLumpNum(lumpindex_t lumpnum, int tag)
{
    ...
    else
    {
        ...
        lump->cache = static_cast<decltype(lump->cache)>(Z_Malloc(W_LumpLength(lumpnum), tag, &lump->cache));
...
int W_LumpDump (const char *lumpname)
{
    ...
    lump_p = static_cast<decltype(lump_p)>(malloc(lumpinfo[i]->size));
    W_ReadLump(i, lump_p);
