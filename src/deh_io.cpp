...
static deh_context_t *DEH_NewContext(void)
{
    ...
    context = static_cast<decltype(context)>(Z_Malloc(sizeof(*context), PU_STATIC, NULL));

    // Initial read buffer size of 128 bytes

    context->readbuffer_size = 128;
    context->readbuffer = static_cast<decltype(context->readbuffer)>(Z_Malloc(context->readbuffer_size, PU_STATIC, NULL));
    context->linenum = 0;
...
deh_context_t *DEH_OpenLump(int lumpnum)
{
    ...
    lump = static_cast<unsigned char *>(W_CacheLumpNum(lumpnum, PU_STATIC));
    ...
    context->filename = static_cast<decltype(context->filename)>(malloc(9));
...
static void IncreaseReadBuffer(deh_context_t *context)
{
    ...
    newbuffer = static_cast<decltype(newbuffer)>(Z_Malloc(newbuffer_size, PU_STATIC, NULL));
