...
static wchar_t *ConvertMultiByteToWide(const char *str, UINT code_page)
{
    ...
    wstr = static_cast<decltype(wstr)>(malloc(sizeof(wchar_t) * wlen));
...
static char *ConvertWideToMultiByte(const wchar_t *wstr, UINT code_page)
{
    ...
    str = static_cast<decltype(str)>(malloc(sizeof(char) * len));
...
int M_vsnprintf(char *buf, size_t buf_len, const char *s, va_list args)
{
    ...
    if (result < 0 || static_cast<unsigned int>(result) >= buf_len)
    {
        buf[buf_len - 1] = '\0';
        result = buf_len - 1;
    }
