...
{
    tinttable = static_cast<decltype(tinttable)>(W_CacheLumpName("TINTTAB", PU_STATIC));
}
...
{
    xlatab = static_cast<decltype(xlatab)>(W_CacheLumpName("XLATAB", PU_STATIC));
}
...
void V_DrawHorizLine(int x, int y, int w, int c)
{
    ...
    if (static_cast<int>(x + w) > SCREENWIDTH)
	w = SCREENWIDTH - x;
...
void V_DrawFullscreenRawOrPatch(lumpindex_t index)
{
    ...
    patch = static_cast<decltype(patch)>(W_CacheLumpNum(index, PU_CACHE));
                  ...
                  byte *palette)
{
    ...
    pcx = static_cast<decltype(pcx)>(Z_Malloc (width*height*2+1000, PU_STATIC, NULL));
