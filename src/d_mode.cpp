...
boolean D_ValidGameVersion(GameMission_t mission, int version)
{
    ...
    for (const auto &valid_version: valid_versions)
    {
        if (valid_version.mission == mission
         && valid_version.version == static_cast<GameVersion_t>(version))
        {
            return true;
        }
    }
