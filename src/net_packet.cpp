...
net_packet_t *NET_NewPacket(int initial_size)
{
    ...
    packet->data = static_cast<decltype(packet->data)>(Z_Malloc(initial_size, PU_STATIC, 0));
    packet->len = 0;
    packet->pos = 0;
...
static void NET_IncreasePacket(net_packet_t *packet)
{
    ...
    newdata = static_cast<decltype(newdata)>(Z_Malloc(packet->alloced, PU_STATIC, 0));
