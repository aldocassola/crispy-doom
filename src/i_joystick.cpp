...
{
    int state;
    SDL_GameControllerButton physbutton;

    // Map from virtual button to physical (SDL) button.
    if (vbutton < NUM_VIRTUAL_BUTTONS)
    {
        physbutton = static_cast<SDL_GameControllerButton>(joystick_physical_buttons[vbutton]);
    }
    else
    {
        physbutton = static_cast<SDL_GameControllerButton>(vbutton);
    }

    switch (static_cast<int>(physbutton))
    {
