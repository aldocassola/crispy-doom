...
z_level_vector<sector_t>	sectors;
...
boolean P_LoadBlockMap (int lump)
{
    ...
    wadblockmaplump = static_cast<decltype(wadblockmaplump)>(Z_Malloc(lumplen, PU_LEVEL, NULL));
    W_ReadLump(lump, wadblockmaplump);
    blockmaplump = static_cast<decltype(blockmaplump)>(Z_Malloc(sizeof(*blockmaplump) * count, PU_LEVEL, NULL));
...
void P_GroupLines (void)
{
    ...
    linebuffer = static_cast<decltype(linebuffer)>(Z_Malloc (totallines*sizeof(line_t *), PU_LEVEL, 0));
...
static void P_LoadReject(int lumpnum)
{
    ...
    minlength = (sectors.size() * sectors.size() + 7) / 8;
    ...
    {
        rejectmatrix = static_cast<decltype(rejectmatrix)>(W_CacheLumpNum(lumpnum, PU_LEVEL));
    }
    else
    {
        rejectmatrix = static_cast<decltype(rejectmatrix)>(Z_Malloc(minlength, PU_LEVEL, &rejectmatrix));
