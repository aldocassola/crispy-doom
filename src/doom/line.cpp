...
line_t::line_t( const maplinedef_t &mld)
    :
    v1( &vertexes[static_cast<unsigned short>( SHORT( mld.v1 ) )] ),
    v2( &vertexes[static_cast<unsigned short>( SHORT( mld.v2 ) )] ),
    dx( v2->x - v1->x ),
    dy( v2->y - v1->y ),
    flags( static_cast<unsigned short>( SHORT( mld.flags ) ) ),
    special( SHORT( mld.special ) ), // [crispy] extended nodes)
    tag( SHORT( mld.tag ) ),
    sidenum{SHORT(mld.sidenum[0]), SHORT(mld.sidenum[1])},
