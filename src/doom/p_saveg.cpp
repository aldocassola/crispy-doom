...
{
    // actionf_p1 acp1;
    str = reinterpret_cast<actionf_t *>(saveg_readp());
}

static void saveg_write_actionf_t(actionf_t *str)
{
    // actionf_p1 acp1;
    saveg_writep(reinterpret_cast<void *>(str));
}
...
static void saveg_read_mobj_t(mobj_t *str)
{
    ...
    // struct mobj_s* snext;
    str->snext = reinterpret_cast<decltype(str->snext)>(saveg_readp()) ;

    // struct mobj_s* sprev;
    str->sprev = reinterpret_cast<decltype(str->sprev)>(saveg_readp()) ;
    ...
    // struct mobj_s* bnext;
    str->bnext = reinterpret_cast<decltype(str->bnext)>(saveg_readp()) ;
    ...
    // mobjinfo_t* info;
    str->info = reinterpret_cast<decltype(str->info)>(saveg_readp()) ;
    ...
    str->target = reinterpret_cast<decltype(str->target)>(saveg_readp()) ;
    ...
    str->tracer = reinterpret_cast<decltype(str->tracer)>(saveg_readp()) ;
...
static void saveg_read_player_t(player_t *str)
{
    ...
    // mobj_t* mo;
    str->mo = reinterpret_cast<decltype(str->mo)>(saveg_readp()) ;
    ...
    // char* message;
    str->message = reinterpret_cast<decltype(str->message)>(saveg_readp()) ;
    ...
    // mobj_t* attacker;
    str->attacker = reinterpret_cast<decltype(str->attacker)>(saveg_readp()) ;
...
void P_ArchiveSpecials (void)
{
    ...
    for (th = thinkercap.next ; th != &thinkercap ; th=th->next)
    {
	if (std::get_if<std::monostate>(&th->function))
	{
