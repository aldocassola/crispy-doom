...
void R_InitSpriteDefs(const char **namelist)
{
    ...
    sprites = static_cast<decltype(sprites)>(Z_Malloc(numsprites *sizeof(*sprites), PU_STATIC, NULL));
  ...
  int			x2 )
{
    ...
    patch = static_cast<decltype(patch)>(W_CacheLumpNum (vis->patch+firstspritelump, PU_CACHE));
...
static void R_DrawLSprite (void)
{
	...
	patch = static_cast<decltype(patch)>(W_CacheLumpNum(lump, PU_STATIC));
    }
