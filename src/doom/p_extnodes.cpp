...
void P_LoadNodes_ZDBSP (int lump, boolean compressed)
{
	...
	outlen = 2.5 * len;
	output = static_cast<decltype(output)>(Z_Malloc(outlen, PU_STATIC, 0));
    ...
    {
	newvertarray = static_cast<decltype(newvertarray)>(Z_Malloc((orgVerts + newVerts) * sizeof(vertex_t), PU_LEVEL, 0));
