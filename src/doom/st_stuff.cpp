...
void ST_refreshBackground(boolean force)
{
	...
	if ((SCREENWIDTH >> crispy->hires) != ST_WIDTH)
	{
		...
		src = static_cast<decltype(src)>(W_CacheLumpName(name, PU_CACHE));
		...
		{
			int x;
			patch_t *const patch = static_cast<decltype(patch)>(W_CacheLumpName(DEH_String("brdr_b"), PU_CACHE));
...
static inline void ST_DrawGibbedPlayerSprites (void)
{
	...
	patch = static_cast<decltype(patch)>(W_CacheLumpNum(sprframe->lump[0] + firstspritelump, PU_CACHE));
...
void ST_drawWidgets(boolean refresh)
{
	...
	if (plyr->readyweapon == wp_fist && plyr->powers[pw_strength])
	{
		...
		patch = static_cast<decltype(patch)>(W_CacheLumpNum(lump, PU_CACHE));
...
void ST_loadData(void)
{
	...
	keys[i] = (lumpnum != -1) ? static_cast<patch_t *>(W_CacheLumpNum(lumpnum, PU_STATIC)) : keys[i-3];
    }
}
