...
constexpr const char *skullName[2] = {"M_SKULL1","M_SKULL2"};
...
static void M_DrawCrispnessBackground(void)
{
	...
	{
		src = static_cast<const byte *>(W_CacheLumpNum(lump, PU_CACHE));
	}
	dest = I_VideoBuffer;
...
void M_Init (void)
{
        ...
        if (crispy->havenerve)
        {
            ...
            if (i != -1 && j != -1)
            {
                ...
                if (W_IsIWADLump(lumpinfo[i]) && W_IsIWADLump(lumpinfo[j]))
                {
                    ...
                    pi = static_cast<decltype(pi)>(W_CacheLumpNum(i, PU_CACHE));
                    pj = static_cast<decltype(pj)>(W_CacheLumpNum(j, PU_CACHE));
	...
	patchl = static_cast<decltype(patchl)>(W_CacheLumpName(DEH_String("M_LOADG"), PU_CACHE));
	patchs = static_cast<decltype(patchs)>(W_CacheLumpName(DEH_String("M_SAVEG"), PU_CACHE));
	patchm = static_cast<decltype(patchm)>(W_CacheLumpName(DEH_String("M_LSLEFT"), PU_CACHE));
