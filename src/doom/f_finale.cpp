...
void F_Ticker (void)
{
	...
	wipegamestate = GS_FLUSH_SCREEN;		// force a wipe
	if (gameepisode == 3)
...
{
	auto castaction = caststate->action;
	auto nextaction = (&states[caststate->nextstate])->action;

	// [crispy] fix Doomguy in casting sequence
	if (std::get_if<std::monostate>(&castaction))
	{
		if (st == S_PLAY_ATK2)
			return sfx_dshtgn;
		else
	...
	{
		for (const auto &as: actionsounds)
		{
...
void F_StartCast (void)
{
    wipegamestate = GS_FLUSH_SCREEN;		// force a screen wipe
    castnum = 0;
...
void F_CastDrawer (void)
{
    ...
    patch = static_cast<patch_t *>(W_CacheLumpNum (static_cast<lumpindex_t>(lump+firstspritelump), PU_CACHE));
    if (flip)
