...
void P_InitPicAnims (void)
{
    ...
    if (from_lump)
    {
	animdefs = static_cast<decltype(animdefs)>(W_CacheLumpName("ANIMATED", PU_STATIC));
    }
    else
    {
...
void P_PlayerInSpecialSector (player_t* player)
{
	...
	if (showMessages && crispy->secretmessage && player == &players[consoleplayer])
	{
	    ...
	    player->centermessage = (crispy->secretmessage == SECRETMESSAGE_COUNT) ? const_cast<char *>(str_count) : const_cast<char *>(HUSTR_SECRETFOUND);
	    if (sfx_id != -1)
