...
static int G_NextWeapon(int direction)
{
    ...
    i = 0;
    for (const auto &weapon_order: weapon_order_table)
    {
        if (weapon_order.weapon == weapon)
        {
            break;
        }
        i++;
...
static void G_AddDemoFooter(void)
{
    ...
    wadinfo_t header;
    std::copy_n("PWAD", 4, header.identification);
...
boolean G_CheckDemoStatus (void)
{
	...
	*demo_p++ = DEMOMARKER;
	...
	const auto *msg = success ? "Demo %s recorded%c" : "Failed to record Demo %s%c";
	Z_Free (demobuffer);
	demorecording = false;
	// [crispy] if a new game is started during demo recording, start a new demo
