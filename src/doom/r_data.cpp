...
void R_GenerateLookup (int texnum)
{
    ...
    for (i=0 , patch = texture->patches;
	 ...
	 i++, patch++)
    {
	realpatch = static_cast<decltype(realpatch)>(W_CacheLumpNum(patch->patch, PU_CACHE));
	x1 = patch->originx;
	x2 = x1 + SHORT(realpatch->width);
...
//  if (texture->patchcount > 1 && texture->height < 256)
	...
	{
	    int pat = patch->patch;
	    const patch_t *realpatch = static_cast<decltype(realpatch)>(W_CacheLumpNum(pat, PU_CACHE));
...
void R_InitTextures (void)
{
	...
	if (!strncasecmp(lumpinfo[i]->name, DEH_String("PNAMES"), 6))
	{
	    ...
	    pnameslumps[numpnameslumps].names = static_cast<decltype(pnameslumps[numpnameslumps].names)>(W_CacheLumpNum(pnameslumps[numpnameslumps].lumpnum, PU_STATIC));
    ...
    name[8] = 0;
    patchlookup = static_cast<decltype(patchlookup)>(Z_Malloc(nummappatches * sizeof(*patchlookup), PU_STATIC, NULL));
    ...
    {
	texturelumps[i].maptex = static_cast<decltype(texturelumps[i].maptex)>(W_CacheLumpNum(texturelumps[i].lumpnum, PU_STATIC));
    ...
    textures = static_cast<decltype(textures)>(Z_Malloc (numtextures * sizeof(*textures), PU_STATIC, 0));
    texturecolumnlump = static_cast<decltype(texturecolumnlump)>(Z_Malloc (numtextures * sizeof(*texturecolumnlump), PU_STATIC, 0));
    texturecolumnofs = static_cast<decltype(texturecolumnofs)>(Z_Malloc (numtextures * sizeof(*texturecolumnofs), PU_STATIC, 0));
    texturecolumnofs2 = static_cast<decltype(texturecolumnofs2)>(Z_Malloc (numtextures * sizeof(*texturecolumnofs2), PU_STATIC, 0));
    texturecomposite = static_cast<decltype(texturecomposite)>(Z_Malloc (numtextures * sizeof(*texturecomposite), PU_STATIC, 0));
    texturecomposite2 = static_cast<decltype(texturecomposite2)>(Z_Malloc (numtextures * sizeof(*texturecomposite2), PU_STATIC, 0));
    texturecompositesize = static_cast<decltype(texturecompositesize)>(Z_Malloc (numtextures * sizeof(*texturecompositesize), PU_STATIC, 0));
    texturewidthmask = static_cast<decltype(texturewidthmask)>(Z_Malloc (numtextures * sizeof(*texturewidthmask), PU_STATIC, 0));
    texturewidth = static_cast<decltype(texturewidth)>(Z_Malloc (numtextures * sizeof(*texturewidth), PU_STATIC, 0));
    textureheight = static_cast<decltype(textureheight)>(Z_Malloc (numtextures * sizeof(*textureheight), PU_STATIC, 0));
    texturebrightmap = static_cast<decltype(texturebrightmap)>(Z_Malloc (numtextures * sizeof(*texturebrightmap), PU_STATIC, 0));
    ...
    texturetranslation = static_cast<decltype(texturetranslation)>(Z_Malloc ((numtextures+1)*sizeof(*texturetranslation), PU_STATIC, 0));
...
void R_InitFlats (void)
{
    ...
    // Create translation table for global animation.
    flattranslation = static_cast<decltype(flattranslation)>(Z_Malloc ((numflats+1)*sizeof(*flattranslation), PU_STATIC, 0));
...
void R_InitSpriteLumps (void)
{
    ...
    spritewidth = static_cast<decltype(spritewidth)>(Z_Malloc (numspritelumps*sizeof(*spritewidth), PU_STATIC, 0));
    spriteoffset = static_cast<decltype(spriteoffset)>(Z_Malloc (numspritelumps*sizeof(*spriteoffset), PU_STATIC, 0));
    spritetopoffset = static_cast<decltype(spritetopoffset)>(Z_Malloc (numspritelumps*sizeof(*spritetopoffset), PU_STATIC, 0));
	...
	patch = static_cast<decltype(patch)>(W_CacheLumpNum (firstspritelump+i, PU_CACHE));
...
static void R_InitTranMap()
{
    ...
    if (lump != -1 && W_LumpLength(lump) == 256*256)
    {
        ...
        tranmap = static_cast<decltype(tranmap)>(W_CacheLumpNum(lump, PU_STATIC));
	...
	unsigned char *playpal = static_cast<decltype(playpal)>(W_CacheLumpName("PLAYPAL", PU_STATIC));

	tranmap = static_cast<decltype(tranmap)>(Z_Malloc(256*256, PU_STATIC, 0));
	{
...
void R_InitColormaps (void)
{
    ...
    colormaps = static_cast<decltype(colormaps)>(W_CacheLumpNum(lump, PU_STATIC));
#else
	byte *playpal;
	int c, i, j = 0;
	byte r, g, b;

	playpal = static_cast<decltype(playpal)>(W_CacheLumpName("PLAYPAL", PU_STATIC));
	...
	{
		byte *const colormap = static_cast<decltype(byte *const colormap)>(W_CacheLumpName("COLORMAP", PU_STATIC));
    ...
    {
	byte *playpal = static_cast<decltype(playpal)>(W_CacheLumpName("PLAYPAL", PU_STATIC));
	char c[3];
	int i, j;
...
void R_PrecacheLevel (void)
{
    ...
    // Precache textures.
    texturepresent = static_cast<decltype(texturepresent)>(Z_Malloc(numtextures, PU_STATIC, NULL));
    ...
    // Precache sprites.
    spritepresent = static_cast<decltype(spritepresent)>(Z_Malloc(numsprites, PU_STATIC, NULL));
