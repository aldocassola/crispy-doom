  ...
  statenum_t	stnum )
{
	...
	std::visit([&player, &psp, &timetogo](auto &&action)
	{
        using T = std::decay_t<decltype(action)>;

        if constexpr (std::is_same_v<T, actionf_v>) {
            std::cout << "P_SetPsprite: called actionf_v\n";
        } else if constexpr (std::is_same_v<T, actionf_p1>) {
            std::cout << "P_SetPsprite: called actionf_p1\n";
        } else if constexpr (std::is_same_v<T, actionf_p2>) {
            std::cout << "P_SetPsprite: called actionf_p2\n";
