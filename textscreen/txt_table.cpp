...
static void TXT_CalcTableSize(TXT_UNCAST_ARG(table))
{
    ...
    row_heights = static_cast<decltype(row_heights)>(malloc(sizeof(int) * rows));
    column_widths = static_cast<decltype(column_widths)>(malloc(sizeof(int) * table->columns));
...
txt_table_t *TXT_NewTable(int columns)
{
    ...
    table = static_cast<decltype(table)>(malloc(sizeof(txt_table_t)));
...
void TXT_SetTableColumns(TXT_UNCAST_ARG(table), int new_columns)
{
    ...
    new_num_widgets = (table->num_widgets / table->columns) * new_columns
                    + (table->num_widgets % table->columns);
    new_widgets = static_cast<decltype(new_widgets)>(calloc(new_num_widgets, sizeof(txt_widget_t *)));
