...
int TXT_UnicodeCharacter(unsigned int c)
{
    ...
    for (const auto &codepage: code_page_to_unicode)
    {
        if (static_cast<const unsigned short>(codepage) == c)
        {
            return i;
        }
        i++;
...
static const char *NameForKey(int key)
{
    ...
    for (const auto &scancode: scancode_translate_table)
    {
        if (scancode == key)
        {
    ...
    for (const auto &keyname: key_names)
    {
        if (keyname.key == key)
        {
            return keyname.name;
